...
- `upstream/README.md`: durable cross-Day reference policy, current pinned
  HEADs, journey-critical files, and licensing/use boundaries.
...
- secrets, cookies, `.env` files, real Steam profile data, and remote fonts.
  Approved runtime Steam CDN cover requests are documented in
  `specs/HITL-DECISIONS.md`.

Run `powershell -ExecutionPolicy Bypass -File scripts/sync-next-save-resources.ps1`
to refresh the selective import and manifest. The script refuses to overwrite a
local file whose SHA-256 no longer matches the previous manifest; resolve that
file deliberately instead of discarding user changes.
